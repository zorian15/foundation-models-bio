...
The 2024–2026 shift is that AlphaFold3-class capability is no longer a single lab's. Open-weight **co-folding** models (predicting the joint structure of a complex, sequence and ligand folded together) now match it closely: **Boltz-1 (MIT) and Boltz-2 (MIT and Recursion)** [@wohlwend2024; @passaro2025] and **Chai-1** from Chai Discovery [@chai2024] both reproduce AlphaFold3-level accuracy under open licenses, and Boltz-2 adds a binding-affinity head that outperformed every method submitted to the CASP16 affinity challenge in a retrospective evaluation. For an ML practitioner, this matters: you can run and fine-tune these yourself.
